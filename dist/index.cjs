    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0f1e; color: #ffffff; padding: 30px; border-radius: 12px;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <tr style="border-bottom: 1px solid #ffffff10;">
          ...
          <td style="padding: 12px; color: #fff;">${o.name}</td>
        </tr>
        <tr style="border-bottom: 1px solid #ffffff10;">
          <td style="padding: 12px; color: #06b6d4; font-weight: bold;">Email</td>
          <td style="padding: 12px; color: #fff;"><a href="mailto:${o.email}" style="color: #38bdf8;">${o.email}</a></td>
        </tr>
        ${o.phone?`<tr style="border-bottom: 1px solid #ffffff10;"><td style="padding: 12px; color: #06b6d4; font-weight: bold;">Phone</td><td style="padding: 12px; color: #fff;">${o.phone}</td></tr>`:""}
        ${o.company?`<tr style="border-bottom: 1px solid #ffffff10;"><td style="padding: 12px; color: #06b6d4; font-weight: bold;">Company</td><td style="padding: 12px; color: #fff;">${o.company}</td></tr>`:""}
        ${o.service?`<tr style="border-bottom: 1px solid #ffffff10;"><td style="padding: 12px; color: #06b6d4; font-weight: bold;">Service</td><td style="padding: 12px; color: #fff;">${o.service}</td></tr>`:""}
        ${o.budget?`<tr style="border-bottom: 1px solid #ffffff10;"><td style="padding: 12px; color: #06b6d4; font-weight: bold;">Budget</td><td style="padding: 12px; color: #fff;">${o.budget}</td></tr>`:""}
        <tr>
          <td style="padding: 12px; color: #06b6d4; font-weight: bold; vertical-align: top;">Message</td>
          <td style="padding: 12px; color: #fff; line-height: 1.6;">${o.message.replace(/\n/g,"<br>")}</td>
        </tr>
      </table>
      <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #ffffff10; text-align: center;">
        <p style="color: #ffffff40; font-size: 12px;">Reply directly to this email to respond to ${o.name}</p>
      </div>
    </div>
  `;await ie.sendMail({from:`"Devoria Tech Website" <${process.env.SMTP_USER||"info@devoriatech.com"}>`,to:process.env.CONTACT_EMAIL||"info@devoriatech.com",replyTo:o.email,subject:`New Inquiry from ${o.name}${o.service?` - ${o.service}`:""}`,html:e})}async function re(){if((await c.getAllBlogs()).length>0)return;let e=[{title:"Why Every Business Needs a Professional Website in 2026",slug:"why-every-business-needs-professional-website-2026",excerpt:"Discover why having a professional website is essential for businesses in 2026. Learn how a modern website builds trust, attracts customers, and improves marketing results.",category:"Web Development",heroImage:"/assets/blog-website-hero.webp",metaTitle:"Why Every Business Needs a Professional Website in 2026 | Devoria Tech",metaDescription:"Discover why having a professional website is essential for businesses in 2026. Learn how a modern website builds trust, attracts customers, and improves marketing results.",metaKeywords:"professional website, business website 2026, website importance, web development, SEO website, mobile-friendly website",published:!0,publishedAt:new Date("2026-03-08"),content:`<h2>Your Website is Your Digital Storefront</h2>
...
<h2>Conclusion</h2>
<p>AI automation is the smartest investment a growing business can make. Devoria Tech builds custom AI automation solutions that save time, reduce costs, and scale with your business.</p>`}];for(let t of e)await c.createBlog(t);console.log("Blog posts seeded successfully")}async function W(o,e){return await re(),e.post("/api/contact",async(t,s)=>{try{let i=k.parse(t.body),r=await c.createContactSubmission(i);try{await ne(i),console.log(`Contact email sent for submission ${r.id}`)}catch(y){console.error("Failed to send contact email:",y)}s.json({success:!0,id:r.id})}catch(i){i instanceof I.ZodError?s.status(400).json({error:"Invalid form data",details:i.errors}):(console.error("Contact form error:",i),s.status(500).json({error:"Failed to submit form"}))}}),e.get("/api/blogs",async(t,s)=>{try{let i=await c.getAllBlogs();s.json(i.filter(r=>r.published))}catch{s.status(500).json({error:"Failed to fetch blogs"})}}),e.get("/api/blogs/:slug",async(t,s)=>{try{let i=await c.getBlogBySlug(t.params.slug);if(!i||!i.published)return s.status(404).json({error:"Blog post not found"});s.json(i)}catch{s.status(500).json({error:"Failed to fetch blog post"})}}),e.get("/api/seo/:pageSlug",async(t,s)=>{try{let i=await c.getSeoByPageSlug(t.params.pageSlug);s.json(i||null)}catch{s.status(500).json({error:"Failed to fetch SEO settings"})}}),e.post("/api/admin/login",async(t,s)=>{let{username:i,password:r}=t.body;i===j&&oe(r)?(t.session.adminAuthenticated=!0,s.json({success:!0})):s.status(401).json({error:"Invalid credentials"})}),e.post("/api/admin/logout",(t,s)=>{t.session.destroy(()=>{s.json({success:!0})})}),e.get("/api/admin/me",(t,s)=>{t.session?.adminAuthenticated?s.json({authenticated:!0,username:j}):s.status(401).json({error:"Not authenticated"})}),e.get("/api/admin/contacts",h,async(t,s)=>{try{let i=await c.getContactSubmissions();s.json(i)}catch{s.status(500).json({error:"Failed to fetch contacts"})}}),e.get("/api/admin/blogs",h,async(t,s)=>{try{let i=await c.getAllBlogs();s.json(i)}catch{s.status(500).json({error:"Failed to fetch blogs"})}}),e.post("/api/admin/blogs",h,async(t,s)=>{try{let i=x.parse(t.body),r=await c.createBlog(i);s.json(r)}catch(i){i instanceof I.ZodError?s.status(400).json({error:"Invalid data",details:i.errors}):(console.error("Create blog error:",i),s.status(500).json({error:"Failed to create blog"}))}}),e.put("/api/admin/blogs/:id",h,async(t,s)=>{try{let i=await c.updateBlog(t.params.id,t.body);s.json(i)}catch(i){console.error("Update blog error:",i),s.status(500).json({error:"Failed to update blog"})}}),e.delete("/api/admin/blogs/:id",h,async(t,s)=>{try{await c.deleteBlog(t.params.id),s.json({success:!0})}catch{s.status(500).json({error:"Failed to delete blog"})}}),e.get("/api/admin/seo",h,async(t,s)=>{try{let i=await c.getAllSeoSettings();s.json(i)}catch{s.status(500).json({error:"Failed to fetch SEO settings"})}}),e.put("/api/admin/seo/:pageSlug",h,async(t,s)=>{try{let i=P.parse({...t.body,pageSlug:t.params.pageSlug}),r=await c.upsertSeoSettings(i);s.json(r)}catch(i){i instanceof I.ZodError?s.status(400).json({error:"Invalid data",details:i.errors}):(console.error("Upsert SEO error:",i),s.status(500).json({error:"Failed to update SEO settings"}))}}),o}var F=g(require("express"),1),z=g(require("fs"),1),C=g(require("path"),1);function V(o){let e=C.default.resolve(__dirname,"public");if(!z.default.existsSync(e))throw new Error(`Could not find the build directory: ${e}, make sure to build the client first`);o.use(F.default.static(e)),o.use("/{*path}",(t,s)=>{s.sendFile(C.default.resolve(e,"index.html"))})}var Y=require("http"),p=(0,B.default)(),L=(0,Y.createServer)(p);p.use(B.default.json({verify:(o,e,t)=>{o.rawBody=t}}));p.use(B.default.urlencoded({extended:!1}));p.use((0,q.default)({secret:process.env.SESSION_SECRET||"devoria-admin-secret-2026",resave:!1,saveUninitialized:!1,cookie:{secure:!0,httpOnly:!0,maxAge:1440*60*1e3}}));function E(o,e="express"){let t=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",second:"2-digit",hour12:!0});console.log(`${t} [${e}] ${o}`)}p.use((o,e,t)=>{let s=Date.now(),i=o.path,r,y=e.json;e.json=function(v,...S){return r=v,y.apply(e,[v,...S])},e.on("finish",()=>{let v=Date.now()-s;if(i.startsWith("/api")){let S=`${o.method} ${i} ${e.statusCode} in ${v}ms`;r&&(S+=` :: ${JSON.stringify(r)}`),E(S)}}),t()});(async()=>{await W(L,p),p.use((e,t,s,i)=>{let r=e.status||e.statusCode||500,y=e.message||"Internal Server Error";return console.error("Internal Server Error:",e),s.headersSent?i(e):s.status(r).json({message:y})}),V(p);let o=parseInt(process.env.PORT||"5000",10);L.listen({port:o,host:"0.0.0.0",reusePort:!0},()=>{E(`serving on port ${o}`)})})();0&&(module.exports={log});
